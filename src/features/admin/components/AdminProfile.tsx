...
import { UserIcon, Shield, Crown, Key, Bell, Save, Upload, Activity, Phone } from "lucide-react"
...
function getDisplayName(
  src: { fullName?: string | null; username?: string | null; email?: string | null },
  fallback: string,
) {
...
export function AdminProfile({ admin = defaultAdmin }: AdminProfileProps) {
  ...
  const effective = useMemo(() => {
    const name = getDisplayName(
      { fullName: apiUser?.fullName, username: apiUser?.username, email: apiUser?.email },
    ...
    return {
      ...
      phone: normalize((apiUser as unknown as ExtendedApiUser)?.phone) ?? (admin as unknown as ExtendedAdminUser)?.phone,
      username: apiUser?.username ?? (admin as unknown as ExtendedAdminUser)?.username,
      companyName: (apiUser as unknown as ExtendedApiUser)?.companyName ?? (admin as unknown as ExtendedAdminUser)?.companyName,
      avatar: admin?.avatar ?? "/placeholder.svg?height=100&width=100",
      role: (admin?.role ?? "admin") as RoleType,
      createdAt: apiUser?.createdAt ?? admin?.createdAt ?? new Date().toISOString(),
  ...
  const initials = (
    effective.name && effective.name.trim().length > 0 ? effective.name : (nameFromEmail(effective.email) ?? "U")
  )
    .split(" ")
    .filter(Boolean)
    .map((n) => n[0])
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Badge className={`bg-gradient-to-r ${getRoleColor(effective.role)} text-white border-0 text-sm`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-6">
                  <div className="relative">
                    <Avatar className="w-16 sm:w-20 h-16 sm:h-20 ring-4 ring-blue-200">
                      <AvatarImage
                        src={effective.avatar || "/placeholder.svg?height=100&width=100&query=admin-avatar"}
                        alt={effective.name}
                      />
                      <AvatarFallback
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg sm:text-xl font-semibold text-blue-600">{effective.name}</h3>
                    <p className="text-slate-600 text-sm sm:text-base">{effective.email}</p>
                    {effective.phone && (
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      <Badge className={`bg-gradient-to-r ${getRoleColor(effective.role)} text-white border-0 text-xs`}>
                        <RoleIcon className="w-3 h-3 mr-1" />
                        {String(effective.role).replace("_", " ").toUpperCase()}
                      </Badge>
                      <span className="text-xs text-slate-600">
                        {t("memberSince")} {new Date(effective.createdAt).toLocaleDateString("fr-FR")}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <Card className="bg-white border-blue-200/30">
            ...
            <CardContent className="space-y-4">
              {(
                [
                  { key: "emailAlerts", label: t("emailAlerts"), description: t("emailAlertsDesc") },
                  { key: "systemAlerts", label: t("systemAlerts"), description: t("systemAlertsDesc") },
                  { key: "userRegistrations", label: t("userRegistrations"), description: t("userRegistrationsDesc") },
                  { key: "criticalErrors", label: t("criticalErrors"), description: t("criticalErrorsDesc") },
                ] as const
              ).map((setting) => (
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-white border-blue-200/30">
            ...
            <CardContent className="space-y-4">
              {[
                { label: t("sessionsToday"), value: "12", color: "text-blue-400" },
                { label: t("actionsPerformed"), value: "47", color: "text-green-400" },
                { label: t("usersManaged"), value: "156", color: "text-purple-400" },
                { label: t("connectionTime"), value: "4h 23m", color: "text-yellow-400" },
              ].map((stat, index) => (
                <motion.div
                  key={stat.label}
