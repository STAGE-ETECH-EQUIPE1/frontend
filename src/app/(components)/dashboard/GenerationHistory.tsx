...
const mockGenerationHistory: GenerationRecord[] = [
  {
    ...
    generatedLogos: [
      { id: "logo-1", url: "/placeholder.svg?height=200&width=200&text=TechCorp1", downloaded: true, favorited: true },
      {
        id: "logo-2",
      ...
      { id: "logo-3", url: "/placeholder.svg?height=200&width=200&text=TechCorp3", downloaded: true, favorited: false },
      { id: "logo-4", url: "/placeholder.svg?height=200&width=200&text=TechCorp4", downloaded: false, favorited: true },
    ],
    tokensUsed: 3,
    status: "completed",
    duration: 45,
  },
  {
    ...
    generatedLogos: [
      { id: "logo-5", url: "/placeholder.svg?height=200&width=200&text=Saveurs1", downloaded: true, favorited: false },
      { id: "logo-6", url: "/placeholder.svg?height=200&width=200&text=Saveurs2", downloaded: false, favorited: true },
      { id: "logo-7", url: "/placeholder.svg?height=200&width=200&text=Saveurs3", downloaded: false, favorited: false },
      { id: "logo-8", url: "/placeholder.svg?height=200&width=200&text=Saveurs4", downloaded: true, favorited: false },
    ],
    tokensUsed: 3,
    status: "completed",
    duration: 38,
  ...
  },
  {
    ...
    generatedLogos: [
      ...
      { id: "logo-14", url: "/placeholder.svg?height=200&width=200&text=EcoVert2", downloaded: true, favorited: false },
    ],
    tokensUsed: 2,
    status: "failed",
    duration: 15,
...
export function GenerationHistory() {
  ...
  const successfulGenerations = history.filter((r) => r.status === "completed").length
  const totalTokensUsed = history.reduce((sum, record) => sum + record.tokensUsed, 0)
  const averageDuration = Math.round(
    history.filter((r) => r.status === "completed").reduce((sum, record) => sum + record.duration, 0) /
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">Consultez et gérez vos générations de logos précédentes</p>
        </div>
        <div className="flex items-center gap-4">
          <Badge className="bg-blue-100 text-blue-700 border-blue-200">
            <History className="w-3 h-3 mr-1" />
            {filteredHistory.length} génération{filteredHistory.length > 1 ? "s" : ""}
          </Badge>
        </div>
      </div>
            ...
            >
              <Card className="bg-white border-slate-200 hover:border-blue-300 transition-all duration-300 shadow-sm hover:shadow-md">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-lg font-semibold text-slate-800">{record.companyName}</h3>
                        <Badge className={getStatusColor(record.status)}>{getStatusText(record.status)}</Badge>
                        <Badge variant="outline" className="text-xs">
                          {record.tokensUsed} token{record.tokensUsed > 1 ? "s" : ""}
                        </Badge>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        <div className="flex items-center gap-1">
                          <span className="text-xs text-slate-600">{record.colorSchemeName}:</span>
                          <div className="flex">
                      ...
                      {record.description && <p className="text-sm text-slate-600 italic">"{record.description}"</p>}
                    </div>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        <DropdownMenuItem onClick={() => regenerateWithSameParams(record)}>
                          <RefreshCw className="w-4 h-4 mr-2" />
                          Régénérer
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => copyParameters(record)}>
                  ...
                  {/* Logos générés */}
                  {record.status === "completed" && record.generatedLogos.length > 0 && (
                    <div className="border-t pt-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-slate-800">Logos générés ({record.generatedLogos.length})</h4>
                        <Dialog>
                          <DialogTrigger asChild>
                            <Button size="sm" variant="outline" onClick={() => setSelectedGeneration(record)}>
                          ...
                          <DialogContent className="max-w-4xl">
                            <DialogHeader>
                              <DialogTitle className="text-blue-600">Logos générés - {record.companyName}</DialogTitle>
                              <DialogDescription>
                                Génération du {new Date(record.timestamp).toLocaleDateString("fr-FR")}
                              </DialogDescription>
                            </DialogHeader>
                            {selectedGeneration && (
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {selectedGeneration.generatedLogos.map((logo) => (
                                  <div key={logo.id} className="group relative">
                                    ...
                                    <div className="flex items-center justify-center gap-2">
                                      <Button
                                        size="sm"
                                        variant="ghost"
                                        className={logo.favorited ? "text-red-500" : ""}
                                      >
                                        <Heart className={`w-4 h-4 ${logo.favorited ? "fill-red-500" : ""}`} />
                                      </Button>
                                      <Button
                                        size="sm"
                                        variant="ghost"
                                        className={logo.downloaded ? "text-green-500" : ""}
                                      >
                      ...
                      {record.status === "completed" && (
                        <>
                          <span className="text-sm text-slate-600">
                            {record.generatedLogos.filter((l) => l.downloaded).length} téléchargé
                            {record.generatedLogos.filter((l) => l.downloaded).length > 1 ? "s" : ""}
                          </span>
                          <span className="text-slate-300">•</span>
                          <span className="text-sm text-slate-600">
                            {record.generatedLogos.filter((l) => l.favorited).length} favori
                            {record.generatedLogos.filter((l) => l.favorited).length > 1 ? "s" : ""}
                          </span>
                        </>
                      )}
                    </div>
      ...
      {filteredHistory.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
          ...
          <div className="text-slate-600 text-lg">Aucune génération trouvée</div>
          <p className="text-sm text-slate-500 mt-2">
            Essayez de modifier vos critères de recherche ou commencez à générer des logos
          </p>
        </motion.div>
      )}
    </motion.div>
