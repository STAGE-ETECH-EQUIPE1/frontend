    ...
    comments: [
      {
        ...
        content: "Très beau logo ! Le style moderne correspond parfaitement à notre startup.",
        rating: 5,
...
export function ProjectsManager() {
  ...
  const addComment = () => {
    ...
    setProjects(
      projects.map((project) =>
        project.id === selectedProject.id ? { ...project, comments: [...project.comments, comment] } : project,
      ),
    )
  ...
  const styles = ["Moderne", "Minimaliste", "Vintage", "Professionnel", "Créatif", "Élégant", "Dynamique", "Classique"]
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 text-sm sm:text-base">Gérez vos logos créés et partagez vos commentaires</p>
        </div>
              ...
              <Card className="bg-white border-slate-200 hover:border-blue-300 transition-all duration-300 group shadow-sm hover:shadow-lg">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div>
                      <CardTitle className="text-slate-800 text-lg">{project.name}</CardTitle>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className={project.isFavorite ? "text-red-500 border-red-200" : ""}
                    >
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        ...
                        >
                          ...
                          <span className="hidden sm:inline truncate">Commentaires</span>
                          <span className="sm:hidden">({project.comments.length})</span>
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-xl sm:max-w-2xl">
                        <DialogHeader>
                          <DialogTitle className="text-blue-600 text-sm sm:text-base">
                            Commentaires - {project.name}
                          </DialogTitle>
                          <DialogDescription className="text-sm sm:text-base">
                            Partagez vos impressions et consultez les avis
                          </DialogDescription>
                        </DialogHeader>
                        {selectedProject && (
                          <div className="space-y-6">
                            ...
                            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-lg">
                              <img
                                src={selectedProject.logoUrl || "/placeholder.svg"}
                              ...
                              <div>
                                ...
                                <p className="text-sm text-slate-600">
                                  {selectedProject.style} • {selectedProject.industry}
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="space-y-4 p-4 bg-blue-50 rounded-lg">
                              <div>
                                ...
                                <div className="flex mt-1">{renderStars(newRating, true, setNewRating)}</div>
                              </div>
                              <div>
                                <label className="text-sm font-medium text-slate-700 text-sm sm:text-base">
                                  Commentaire
                                ...
                                <Textarea
                                  ...
                                  placeholder="Partagez votre avis sur ce logo..."
                            ...
                            <div className="space-y-4 max-h-96 overflow-y-auto">
                              {selectedProject.comments.length > 0 ? (
                                selectedProject.comments.map((comment) => (
                                  <div key={comment.id} className="flex gap-3 p-4 bg-slate-50 rounded-lg">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <p className="text-sm text-slate-700 text-sm sm:text-base">{comment.content}</p>
                                    </div>
                                  </div>
                                ))
                              ) : (
                                <div className="text-center py-8 text-slate-500">
                                  ...
                                  <p className="text-sm text-sm sm:text-base">Soyez le premier à donner votre avis !</p>
                                </div>
                              )}
                            </div>
                          </div>
      ...
      {filteredProjects.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
          ...
          <div className="text-slate-600 text-lg text-sm sm:text-base">Aucun projet trouvé</div>
          <p className="text-sm text-slate-500 mt-2 text-sm sm:text-base">
            Essayez de modifier vos critères de recherche ou créez votre premier logo
          </p>
        </motion.div>
      )}
    </motion.div>
